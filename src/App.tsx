...
export default function App() {
        ...
        >
          <Star
            className={cn(
              ...
              isFav ? "fill-yellow-400 text-yellow-400" : "text-slate-300 hover:text-yellow-400"
            )}
          />
        </button>
        {/* Delete */}
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-28">
      ...
      {/* ─── FAB Scan Button (dashboard only) ─────────────────────────────── */}
      {view === "dashboard" && (
        <motion.div
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: 0.6, type: "spring", stiffness: 260, damping: 20 }}
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-3rem)] max-w-sm"
        >
          <button
            onClick={() => {
            ...
            className="w-full h-16 bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white rounded-[2rem] font-black text-lg shadow-2xl shadow-orange-300/60 flex items-center justify-center gap-3 transition-all active:scale-[0.97]"
          >
          ...
          {selectedMealForView !== null && (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                {/* History section in meal view */}
                <div className="space-y-4 pt-4 border-t">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Historique / Favoris</h4>
                    {productHistory.length > 0 && (
                      <button className="text-[10px] text-slate-400 hover:text-red-500 font-bold"
                        onClick={() => setProductHistory([])}>
                        Vider
                      </button>
                    )}
